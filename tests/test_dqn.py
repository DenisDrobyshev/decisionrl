...
def test_qrdqn_constructs_and_predicts(quiet_logger):
    agent = QRDQN(make_env(), n_quantiles=21, learning_starts=10, batch_size=8,
                  buffer_size=1000, seed=0, logger=quiet_logger)
    action = agent.predict(np.eye(16, dtype=np.float32)[0])
    assert 0 <= action < 4


@pytest.mark.slow
def test_qrdqn_learns_gridworld(quiet_logger):
    agent = QRDQN(make_env(), n_quantiles=51, learning_rate=1e-3, learning_starts=500,
                  batch_size=64, buffer_size=10_000, hidden_sizes=(64, 64),
