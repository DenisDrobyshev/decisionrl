...
Reads a metrics CSV written by :class:`~decisionrl.utils.Logger` and serves a
self-contained web page that auto-refreshes one live chart per metric
(reward, loss, ...). Launch with ``decisionrl dashboard run.csv`` and open the
...
<title>decisionrl dashboard</title><script>__PLOTLYJS__</script>
...
<body><h1>decisionrl &mdash; live training dashboard</h1><div id="grid"></div>
<script>
...
def create_app(csv_path: str, interval_ms: int = 2000):
    ...
    app = Flask("decisionrl-dashboard")
...
def run_dashboard(csv_path: str, host: str = "127.0.0.1", port: int = 8050,
    ...
    print(f"decisionrl dashboard: http://{host}:{port}  (reading {csv_path})")
