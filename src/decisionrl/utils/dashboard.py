...
per metric). Accepts a :class:`~decisionrl.utils.logger.HistoryLogger`, a metrics
dict ``{name: [(step, value), ...]}``, or a path to a CSV written by
:class:`~decisionrl.utils.logger.Logger`.
"""
...
def plot_dashboard(history, path: str = "dashboard.html",
                   title: str = "decisionrl training dashboard") -> str:
